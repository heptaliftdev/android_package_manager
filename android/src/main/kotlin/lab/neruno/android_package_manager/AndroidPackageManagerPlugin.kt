...
//    }
    ...
    private fun getInstalledPackages(call: MethodCall, result: Result) {
        runWithFlags(
            ...
            resultBuilder = { flags -> packageManager.getInstalledPackages(PackageManager.GET_META_DATA).filter { (it.applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM) == 0  }.map {
                it.toMap()
            }},
            api33ResultBuilder = { flags -> packageManager.getInstalledPackages(PackageManager.GET_META_DATA).filter { (it.applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM) == 0  }.map {
                it.toMap()
            }}
        )
    }
