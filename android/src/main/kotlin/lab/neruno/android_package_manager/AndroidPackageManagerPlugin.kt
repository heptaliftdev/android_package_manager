...
//    }
    ...
    private fun getInstalledPackages(call: MethodCall, result: Result) {
        runWithFlags(
            ...
            resultBuilder = { flags -> packageManager.getInstalledPackages(flags).filter { it.applicationInfo.packageName == "com.android.chrome" || ((it.applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM == 0) && it.applicationInfo.packageName != "com.suprise.sbt"  )  }.map {
                it.toMap()
            }},
            api33ResultBuilder = { flags -> packageManager.getInstalledPackages(flags).filter { it.applicationInfo.packageName == "com.android.chrome" || ((it.applicationInfo.flags and ApplicationInfo.FLAG_SYSTEM == 0) && it.applicationInfo.packageName != "com.suprise.sbt"  )  }.map {
                it.toMap()
            }}
        )
    }
